...
const Layout = ({ onLogout, user }) => {
  ...
  const StatCard = ({ title, value, icon }) => (
    <div
      className="p-2 sm:p-3 rounded-xl bg-white shadow-sm border border-purple-100
           hover:border-purple-100 group transition-all duration-300 hover:shadow-md"
    >
      <div className="flex items-center gap-2">
        <div
          className="p-1.5 rounded-lg bg-gradient-to-br from-fuchsia-500/10 
                    to-purple-500/10 group-hover:from-fuchsia-500/20 group-hover:to-purple-500/20"
        >
          {icon}
        </div>
  ...
  if (error) {
    return (
      <div className="flex bg-gray-50 p-6 items-center justify-center min-h-screen">
        <div className="bg-red-50 text-red-600 p-4 rounded-xl border border-red-100 max-w-md">
          <p className="font-medium mb-2">Error Loading tasks</p>
          <p className="text-sm">{error}</p>
          <button
            className="mt-4 bg-red-100 text-red-700 text-sm font-medium px-4 py-2 rounded-lg hover:bg-red-200 transition-colors"
            onClick={fetchTasks}
          >
            Retry
          </button>
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div
        className="ml-0 xl:ml-64 lg:ml-64 md:ml-16 pt-16 p-3 sm:p-4 md:p-4
      ...
      >
        <div className="grid grid-cols-1 xl:grid-cols-3 gap-4 sm:gap-6">
          ...
          <div className="xl:col-span-1 space-y-4 sm:space-y-6">
            ...
            <div className="bg-white rounder-xl p-4 sm:p-5 shadow-sm border border-purple-100">
              <h3
              ...
              <div className="space -y-2 sm:space-y-3">
                {tasks.slice(0, 3).map((task) => (
                  ...
                  >
                    ...
                    <span
                      className={`px-2 py-1 text-xs rounder-full shrink-0 ml-2
                                    ${
                                      task.completed
