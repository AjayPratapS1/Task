...
const TaskItem = ({
  ...
  const handleDelete = async () => {
    try {
      await axios.delete(`${API_BASE}/tasks/${task._id}/gp`, {
        headers: getAuthHeaders(),
      });
      onRefresh?.();
    } catch (err) {
  ...
  const handleSave = async (updatedTask) => {
    try {
      ...
      await axios.put(`${API_BASE}/tasks/${task._id}/gp`, payload, {
        headers: getAuthHeaders(),
      });
              ...
              <div className={TI_CLASSES.menuDropdown}>
                {MENU_OPTIONS.map((opt) => (
                  <button
                    ...
                    className="w-full px-3 sm:px4 py-2 text-left text-sm sm:text-sm hover:bg-purple-50 flex items-center gap-2 transition-colors duration-200"
                  >
                    {opt.icon}
                    {opt.label}
                  </button>
